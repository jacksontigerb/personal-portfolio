...
const SCENES = {
  ...
  creator: {
    ...
    near(p, w, h, f) {
      ...
      const rx = Math.round(w * .38), ry = f - 34;
  ...
  allrounder: {
    ...
    near(p, w, h, f) {
      ...
      const cx = Math.round(w / 2), wy = f - 12;
...
export function paintScene(far, near, key, w, h, floor) {
  const scene = SCENES[key], emitters = [];
  scene.far(painter(far, w, h), w, h, floor);
  scene.near(painter(near, w, h), w, h, floor, emitters);
